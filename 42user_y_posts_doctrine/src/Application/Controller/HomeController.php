...
class HomeController
{
    ...
    public function find_by_username($username)
    {
        $container = ContainerBuilder::buildDevContainer();
        $doctrine = $container->get(Doctrine::class);
        $user = $doctrine->em->getRepository('Application\Models\Entities\User')->findByUsername($username);
        if($user != null){
            echo $this->_formatUser($user);
        }
        else
    ...
    public function insert_post($id)
    {
        ...
        $post->setUser($user);
        $post->setTitle("Nuevo post para Juan");
        $post->setBody("Nuevo body para este post de Juan");
        $doctrine->em->persist($post);
        $doctrine->em->flush();
    }

    public function find_user_with_posts($id)
    {
        $container = ContainerBuilder::buildDevContainer();
        $doctrine = $container->get(Doctrine::class);
        $user = $doctrine->em->find('Application\Models\Entities\User', $id);
        ...
        foreach ($user->getPosts() as $post)
        {
            echo sprintf(
                "%s, %s, %s, %s <br>",
                $post->getId(), $post->getTitle(), $post->getBody(), $post->getCreated()->format("d/m/Y")
            );
        }
    }
